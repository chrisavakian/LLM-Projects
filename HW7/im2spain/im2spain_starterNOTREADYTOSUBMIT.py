...
def grid_search(train_features, train_labels, test_features, test_labels, is_weighted=False, verbose=True):
    ...
    for k in ks:
        ...
        for i, nearest in enumerate(indices):
            ...
            #TODO: FIX bc copy paste
            preds = train_labels[nearest]
            displacements = np.sqrt(((preds[:, 0] - y[0]) * 69) ** 2 + ((preds[:, 1] - y[1]) * 52) ** 2)
            if is_weighted:
                weights = 1 / (distances[i] + 1e-8)  # Avoid division by zero
                e = np.average(displacements, weights=weights)
            else:
                e = np.mean(displacements)
            #TODO: END OF CODE
...
def main():
    ...
    #TODO: Fix this becuase copy paste
    ...
    print("Coordinates of Nearest Images:", train_labels[c[0]])
    ...
    a = np.where(test_files == '53633239060.jpg')[0][0]  # Assuming test_files contains filenames
    b, c = knn.kneighbors([test_features[a]])

    d = train_files[c[0]]
    e = train_labels[c[0]]
    print("Nearest Images:", d)
    print("Coordinates of Nearest Images:", e)

    # Part D: establish a naive baseline of predicting the mean of the training set


    a, b = np.mean(train_labels, axis=0)
    c = np.full(test_labels.shape, [a, b])
    d = np.mean(np.sqrt(((test_labels[:, 0] - a) * 69) ** 2 + ((test_labels[:, 1] - b) * 52) ** 2))
    ...
    for r in ratios:
        ...
        subset_train_features = train_features[:num_samples]
        subset_train_labels = train_labels[:num_samples]

        # Linear Regression
        lr_model = LinearRegression()
        lr_model.fit(subset_train_features, subset_train_labels)
        lr_predictions = lr_model.predict(test_features)
        e_lin = np.sqrt(np.mean(np.square(test_labels - lr_predictions)))

        # k-NN using the best k from previous grid search
        knn_model = NearestNeighbors(n_neighbors=5).fit(subset_train_features, subset_train_labels)  # Example: using k=5
        b, c = knn_model.kneighbors(test_features)
        predicted_labels = np.array([subset_train_labels[idx].mean(axis=0) for idx in c])
        e_nn = np.sqrt(np.mean(np.square(test_labels - predicted_labels)))
